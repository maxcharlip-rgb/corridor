...
// --- marketing homepage stays a light illustrated landing page --------------
{
  ...
  check('editorial asides sit in the side gutters', /class="gutter"/.test(landing) && /48 hours\./.test(landing) && /Invoice after you see the cut\./.test(landing) && /Metro Detroit\./.test(landing) && /From the photos you already have\./.test(landing));
  check('metro Detroit is home, photos from anywhere still get a cut', /Metro Detroit\./.test(landing) && /Send listing photos from anywhere/.test(landing) && /we still cut the tour/.test(landing) && !/Metro Detroit brokers, first/.test(landing) && !/we.ll tell you if we can.t/.test(landing));
