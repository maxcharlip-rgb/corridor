...
// --- marketing homepage stays a light illustrated landing page --------------
{
  ...
  check('hero is a CSS background, not a pixelated or srcset image', /background-size:\s*auto\s*100%/.test(landing) && !/background-size:\s*175%\s*auto/.test(landing) && !/image-rendering:\s*pixelated/.test(landing) && !/srcset/.test(landing));
  check('hero height-fills so the painted river stays in frame', /background-size:\s*auto\s*100%/.test(landing) && /background-position:\s*46%\s*100%/.test(landing) && /image-rendering:\s*auto/.test(landing) && !/street-patch/.test(landing));
